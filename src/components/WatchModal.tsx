...
export default function WatchModal({ item, onClose, initialSeason, initialEpisode }: Props) {
  ...
  // Record this watch + simulate progress tracking
  useEffect(() => {
    recordWatch(item, isTV ? season : undefined, isTV ? episode : undefined);
    startTimeRef.current = Date.now();
    // Simulate progress incrementing while modal is open (90 min movie ≈ 5400s avg)
  ...
  useEffect(() => {
    ...
    return clearAll;
  }, [srcIdx, season, episode]);

  // Detect user clicked inside iframe = video playing
  useEffect(() => {
    const onBlur = () => {
      ...
      <div className="relative flex-1 bg-black">
        ...
        {phase === "failed" && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 bg-black px-6 text-center">
            ...
            <p className="text-white/40 text-sm max-w-xs">All sources were tried automatically. This title may not be available yet.</p>
